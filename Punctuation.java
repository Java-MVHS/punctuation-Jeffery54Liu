...
Take a string of any size, and output all the words with a punctuation in it.
*/
...
public class Punctuation
{
    ...
    {       
        Punctuation p = new Punctuation();                                                                 //new instance of Punctuation
        p.searchIt();                                                                                      //call searchIt()
    }
    
    public void searchIt()
    {                                                                                                      //create the string with all the paragraphs, correcting the format
        String text1 =  "Blood, Sweat, and Tears by Winston Churchill " +
    "May 13, 1940" +
    "Mr. Speaker:" +
    ...
    "But I take up my task with buoyancy and hope. I feel sure that our cause will not be suffered to fail among men. At this time I feel entitled to claim the aid of all, and I say, \"Come then, let us go forward together with our united strength.\"";
        System.out.print("\n\n\n");
        System.out.println(checkForPunctuation(text1));                                                    //calls checkForPunctuation and prints out all words with punctuation
        System.out.print("\n\n\n");
    }
    
    public String checkForPunctuation(String textIn)
    {
        String stored = "";                                                                                //stored will concat all the words with punctuation, which is the final output


        int start = 0;                                                                                         //start will keep track of the index that we started counting from every single word
        int index = 0;                                                                                         //index will keep track where we are in the textIn

        while (index <= textIn.length()-1) {                                                                   //make sures index is in range
            if (textIn.charAt(index) == ' ') {  
                while (index <= textIn.length()-1 && textIn.charAt(index) == ' ') {                            //when the character is a space, and in range, increment index
                    index++;
                }
                if (index > textIn.length()-1) {                                                               //break function when index passes the string's length
                   break;
                }
                start = index;                                                                                 //reset start to the current index As we progress through the string, each word's starting point is at different sections of the string
            }
            else {
                if(textIn.charAt(index) == ',' || textIn.charAt(index) == '.' || textIn.charAt(index) == '?' || //If punctuation is found, concat the word into stored, from the variable start to the index we are on plus 1
                textIn.charAt(index) == '!' || textIn.charAt(index) == ';' || textIn.charAt(index) == ':')
                {
                    if (stored.equals("")) {                                                                    //don't print a space in beginning of the output
                        stored = stored.concat(textIn.substring(start, index+1));
                    }
                    else {
                        stored = stored.concat(" " +  textIn.substring(start, index+1));
                    }
